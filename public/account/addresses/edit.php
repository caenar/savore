...
if (!isset($_SESSION['user'])) {
    header('Location: /auth/login.php?redirect=/account/addresses/edit.php');
    exit;
}
...
$address = new Address();
$userAddresses = $address->getByUser($userId);
...
if (!$currentAddress) {
    header('Location: /account/addresses.php?error=Address+not+found');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $address->update($addressId, $userId, $data);
        header('Location: /account/addresses.php?success=Address+updated+successfully');
        exit;
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
...
<head>
  ...
  <title>Edit Address | Savore</title>
